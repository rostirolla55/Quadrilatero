...
def generate_language_switcher(page_id, current_lang):
    ...
    for lang in LANGUAGES:
        target_file = f"{page_id}-{lang}.html" # Forza sempre il suffisso lingua
...
def update_html_files(repo_root, page_id, nav_key_id, translations, page_title_it):
    ...
    # 1. CREAZIONE FILE CON SUFFISSO LINGUA (it incluso)
    for lang in LANGUAGES:
    ...
    for filename in all_files:
        ...
        try:
            ...
            if f'id="{nav_key_id}"' not in content:
                target_href = f"{page_id}-{current_lang}.html" # Allineato a bsmariamaggiore-it.html
