...
def update_main_js(repo_root, page_id, nav_key_id, lat, lon, distance):
    ...
    # Linee da iniettare: DEFINITE SENZA VIRGOLA FINALE per default
        ...
        for i, line in enumerate(lines):
            ...
            if POI_MARKER in line:
                if not any(f"id: '{page_id}'" in l for l in lines):
                    # CORREZIONE SINTASSI: Se la riga precedente non ha la virgola, la aggiungiamo
                    if i > 0 and '}' in new_lines[-1] and not new_lines[-1].strip().endswith(','):
                        new_lines[-1] = new_lines[-1].rstrip() + ',\n'
                    
                    new_lines.append(new_poi + '\n')
                    print(f"✅ Inserito POI in main.js")
                new_lines.append(line) # Mantiene il marker
