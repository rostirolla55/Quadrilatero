...
def update_main_js(repo_root, page_id, nav_key_id, lat, lon, distance):
    """Aggiorna main.js con logica riga per riga per gestire correttamente le virgole."""
    ...
    for i, line in enumerate(lines):
        if POI_MARKER in line:
            # Controlla la riga precedente per la virgola
            if len(new_lines) > 0:
    ...
    print("✅ main.js aggiornato (riga per riga).")
...
def update_html_files(repo_root, page_id, nav_key_id, translations, page_title_it):
    ...
    for filename in all_files:
        ...
        # Identifica la lingua del file su cui stiamo lavorando
        file_lang = "it"
        for l in LANGUAGES:
        ...
        if f'id="{nav_key_id}"' not in content:
            # Il link nel menu deve puntare alla versione della lingua corrispondente
...
def main():
    ...
    print(f"Aggiornamento V3.9.3 in corso per {page_id}...")
